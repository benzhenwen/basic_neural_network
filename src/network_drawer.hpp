...
public:
    ...
    static void draw(Network & n, glm::vec2 position, glm::vec2 dimentions, GLFWE::Text::CharacterSet & text_writer) {
        ...
        for (Network::Layer & l : n.network) if (tallest_layer_size < l.size()) tallest_layer_size = l.size();

        const int network_layer_count = n.network.size();
        ...
        int previous_layer_size = n.network[0].size();
        float prev_x = horizontal_position(0);

        for (int layer_i = 1; layer_i < n.network.size(); layer_i++) {
            Network::Layer & layer = n.network[layer_i];
            ...
            for (int prev_i = 0; prev_i < previous_layer_size; prev_i++) {
                ...
                for (int curr_i = 0; curr_i < layer_size; curr_i++) {
                    ...
                    GLFWE::Shape::ShapeShader::set_color(line_color(layer.nodes[curr_i].weights[prev_i]));
                    GLFWE::Shape::Quad::draw_line(prev_position, curr_position, line_thickness);

                    // int weight = std::round(layer.nodes[curr_i].weights[prev_i] * 100);
        ...
        // draw nodes on top
        for (unsigned int layer_i = 0; layer_i < n.network.size(); layer_i++) {
            int layer_size = n.network[layer_i].size();
            float curr_x = horizontal_position(layer_i);

            for (int curr_i = 0; curr_i < layer_size; curr_i++) {
                glm::vec2 position = {curr_x, vertical_position(layer_size, curr_i)};
                float node_value = n.network[layer_i].is_input_layer ? n.network[layer_i].nodes[curr_i].raw() : n.network[layer_i].nodes[curr_i].value();
                ...
                int node_bias = std::round(n.network[layer_i].nodes[curr_i].bias * 100);
